# Student Shop App

ASP.NET Core MVC app backed by `shop.db` at the repository root. It uses `Microsoft.Data.Sqlite` directly and stores the selected `customer_id` in a cookie instead of using authentication.

## Routes

- `GET/POST /select-customer`
- `GET /dashboard`
- `GET/POST /place-order`
- `GET /orders`
- `GET /orders/{orderId}`
- `GET /warehouse/priority`
- `POST /scoring/run`

## Prerequisites

- .NET SDK 10.0 or newer
- Python 3 available as `python3`

## Run

```bash
dotnet restore StudentShopApp/StudentShopApp.csproj
dotnet run --project StudentShopApp/StudentShopApp.csproj
```

Open the app on the local ASP.NET Core URL and start with `/select-customer`.

## Key Files

- App project: `StudentShopApp/`
- SQLite database: `shop.db`
- Python scoring job: `jobs/run_inference.py`
- App configuration: `StudentShopApp/appsettings.json`

## Migration Note

When you move to Supabase/Postgres later, keep the controllers and views intact and replace the repository behind `IShopRepository`. That gives you a clean seam for swapping the data store.
